...
func TestTree(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		//	Data:     tVal,
		//	Error: func(t *testing.T, checkErr, execErr error, tp types.Type) {
		//		require.Error(t, checkErr, execErr.Error())
		//	},
		//},
		// {"map .WRONG type", "{{.MII.one}}", "", tVal, false},
		{
			...
			Data:     tVal,
			Error: func(t *testing.T, checkErr, execErr error, tp types.Type) {
				require.ErrorContains(t, execErr, `template:1:6: executing "template" at <.MII.one>: can't evaluate field one in type map[int]int`)
			},
		},
